...
class MCM_Classifier:
    ...
    def __init__(
        ...
        Args:
            - n_categories (int): The number of categories in the dataset -> here 10 digits
            - n_variables (int): The number of variables in the dataset -> 128 pixels here
            - mcm_filename_format (str): The format of the MCM filenames -> .dat file
    ...
    def fit(self,
        ...
        # RUN SA, on each of the 10 MCM for each category of digit
    ...
    def __construct_P(self) -> tuple:
        ...
        for k in range(self.n_categories):
            # For each category of digit, we do the following
            # 1. load a preexisting mcm model? FIXME: how? where do we get these from?
            # 2. 
            ...
            for icc in mcm:
                idx = [i for i in range(self.n_variables) if icc[i] == "1"] # -> [0,1,2,3,...,127] worst case, could also be subset e.g., [0,52,89,103]
                rank = len(idx)

                p_icc = np.zeros(2**rank)  # FIXME: what is going on here? (and below)?
                icc_data = data[:, idx]
                icc_strings = [
    ...
    # choose arguments to put into SA
